...
function createArrayOfSize(totalPages) {
  ...
  function Pagination({totalPages,handlePageChange,currentPage}) {
    ...
    return <HStack spacing={'1rem'} justifyContent='left' alignItems={'center'}><Button onClick={()=>handlePrev(currentPage-1)}>Prev</Button>{pages}<Button onClick={()=>handleNext(currentPage+1)}>Next</Button></HStack>;
  }
